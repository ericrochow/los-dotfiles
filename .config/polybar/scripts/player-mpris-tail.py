...
class PlayerManager:
    ...
    def onMPRISSignal(
        self, interface, properties, signature, member=None, sender=None
    ):
    ...
    def busNameIsAPlayer(self, bus_name):
        return (
            bus_name.startswith("org.mpris.MediaPlayer2")
            and bus_name.split(".")[-1] not in self.blacklist
        )
    ...
    def _parseMetadata(self):
        if self._metadata != None:
            ...
            self.metadata["album"] = re.sub(
                SAFE_TAG_REGEX,
                """\1\1""",
                _getProperty(self._metadata, "xesam:album", ""),
            )
            self.metadata["title"] = re.sub(
                SAFE_TAG_REGEX,
                """\1\1""",
                _getProperty(self._metadata, "xesam:title", ""),
            )
            self.metadata["track"] = _getProperty(
            ...
            self.metadata["disc"] = _getProperty(
                self._metadata, "xesam:discNumber", ""
            )
            self.metadata["date"] = re.sub(
                SAFE_TAG_REGEX,
                """\1\1""",
                _getProperty(self._metadata, "xesam:contentCreated", ""),
            )
            self.metadata["year"] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", self.metadata["date"][0:4]
            )
...
if args.command is None:
    ...
    if args.command == "play" and current_player:
        ...
        print(
            "{} : {}".format(
                current_player.bus_name.split(".")[-1], current_player.status
            )
        )
